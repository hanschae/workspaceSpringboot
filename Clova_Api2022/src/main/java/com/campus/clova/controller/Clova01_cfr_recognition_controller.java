...
public class Clova01_cfr_recognition_controller {
	...
	public String cfrOk(@RequestParam("image") MultipartFile file, HttpSession session) {
		...
		//////////////////////////////
	        ...
	        String filename = null; // 업로드한 파일명 = 네이버 클라우드로 보낼 파일명
	        try {
	        	// 클라이언트의 이미지를 서버에 업로드
	        	filename = ClovaFileupload.fileUpload(path, file);
	        	
	            String paramName = "image"; // 파라미터명은 image로 지정
	            String imgFile = path+"/"+filename; // 서버에 실제 있는 파일의 절대 주소
	            ...
	            String inputLine;
	            if(br != null) {
	                // StringBuffer response = new StringBuffer(); 위로

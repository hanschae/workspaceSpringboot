...
function mediaStart(){
	...
	if(navigator.mediaDevices){ // navigator.mediaDevices : 정보가 있으면
		...
		navigator.mediaDevices.getUserMedia(constraints).then(stream=>{ // 정상 구현시
			...
			// 녹음이 완료되면 서버로 녹음내용을 보내고 결과를 받아 표시
			mediaRecorder.onstop = e => {
				// 바이너리코드로 변환한 후 서버로 보냄
				const blob = new Blob(chunks, {
					'type':'audio/ogg codecs=opus'
				});
				
				// 바이너리코드로 변환된 음성파일을 폼에 추가해서 서버로 보냄
				let formdata = new FormDl wnata();
				formdata.append("audioFile", blob); // <input type="file" name="data"/>

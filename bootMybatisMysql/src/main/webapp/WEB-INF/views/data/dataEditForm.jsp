...
<style>
	.del{
		cursor:pointer; /* 링크 걸린 곳 마우스 모양 변경 */
	}
</style>
<script>
	$(function(){
			...
			<li>
				...
				<div>
					...
					<c:if test="${vo.filename2!=null && vo.filename2!=''}">
						...
						<!-- 첨부파일의 갯수 -->
						<input type="hidden" name="fileCount" id="del" value="2"/>
					</c:if>
					<!-- 첨부파일이 없을 때 -->
					<c:if test="${vo.filename2==null || vo.filename2==''}">
						<input type="file" name="filename" id="filename2"/>
						<input type="hidden" name="fileCount" id="del" value="1"/>
					</c:if>
				</div>
			</li>

...
<script>
	$(function(){
		...
		// 선택한 주소를 회원가입폼으로 세팅하고 현재 창은 닫는다
		$(".zip").click(function(){
...
<div class="">
	...
	<ul id="zipList">
		<c:forEach var="zipVo" items="${zipList}">
			<li>${zipVo.zipcode }</li>
			<li class='zip'>${zipVo.sido }&nbsp;${zipVo.doro }&nbsp;${zipVo.buildname }&nbsp;${zipVo.buildnum1 }<c:if test="${zipVo.buildnum2!='0'}">-${zipVo.buildnum2 }</c:if>&nbsp;(${zipVo.dong },${zipVo.num1 }<c:if test="${zipVo.num2!='0'}">-${zipVo.num2 }</c:if>)</li>
		</c:forEach>
	</ul>
</div>
